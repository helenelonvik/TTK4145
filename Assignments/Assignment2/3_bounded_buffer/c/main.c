...
struct BoundedBuffer* buf_new(int size){
    ...
    // int sema_init(sema_t *sp, unsigned int value, UNSYNC_THREAD, void *NULL);
    sem_init(&buf->full,  0, size);// full when buffer is size
	sem_init(&buf->empty, 0, 0);//, /*starting value?*/); empty when 0
